...
class RentalService:
    def __init__(self, rental_repo: MemoryRepository, book_repo: MemoryRepository, client_repo: MemoryRepository, undo_redo_service):
    ...
    def return_book(self, rental_id: int):
        rental = self.__rental_repo.__getitem__(rental_id)
        new_rental = Rental(rental_id, rental.book, rental.client, rental.rented_date, date.today())
        self.__rental_repo.update(new_rental)
        self.__book_repo[rental.book.id].set_rental_status(False)
        
        redo = FunctionCall(self.__rental_repo.update, new_rental)
        undo = FunctionCall(self.__rental_repo.update, rental)
        redo_book = FunctionCall(self.__book_repo[rental.book.id].set_rental_status, False)
        undo_book = FunctionCall(self.__book_repo[rental.book.id].set_rental_status, True)
        
        operation_rental = Operation(undo, redo)
        operation_book = Operation(undo_book, redo_book)
        
        operations = [operation_rental, operation_book]
        self._undo_redo_service.recordOperation(CascadingOperation(operations))
    ...
    def add_rental(self, rental_id: int, book_id: int, client_id: int, rented_date: date, returned_date: date):
        ...
        redo = FunctionCall(self.__rental_repo.add, rental)
        undo = FunctionCall(self.__rental_repo.remove, int(rental_id))
