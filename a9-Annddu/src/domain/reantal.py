...
class Rental(IdObject):
    ...
    def __str__(self):
        if self.__end > date.today():
            ...
            return f"{self._id} ->  Book id: {self.__book_id._id} -> Title: {self.__book_id.title} | Author: {self.__book_id.author}  |  Client id: {self.__client_id}  |  Start: {self.__start}  |  End: {self.__end}  |  Status: Returned"
